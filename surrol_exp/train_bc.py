...
def main():
    ...
    env = FlattenDictWrapper(env, OBS_DICT)
    env.seed(args.seed)
    set_seed_everywhere(args.seed)

    exp_name = 'bc-' + args.env + '-s' + str(args.seed)
    ...
    reward_before_training, sr = evaluate_policy(bc_trainer.policy, env, video, 0, L, n_eval_episodes=20)
    print(f"Reward/SR before training: {reward_before_training}/{sr}")

    for i in range(50):
        bc_trainer.train(n_epochs=10)
        reward_after_training, sr = evaluate_policy(bc_trainer.policy, env, video, i+1, L, n_eval_episodes=20)
