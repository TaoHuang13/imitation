...
def main():
    ...
    exp_name = 'gail-' + args.env + '-s' + str(args.seed)
    ...
    reward_before_training, sr = evaluate_policy(gail_trainer.policy, env, video, 0, L, n_eval_episodes=20)
    print(f"Reward/SR before training: {reward_before_training}/{sr}")

    for i in range(50):
        gail_trainer.train(2000)
        reward_after_training, sr = evaluate_policy(gail_trainer.policy, env, video, i+1, L, n_eval_episodes=20)
